...
function OutputTab({ item, allOutputItems }: OutputTabProps) {
  ...
  function handleChanging(direction: "next" | "prev") {
    ...
    if (direction === "next") {
      ...
      newIndex =
        (currentIndex - 1 + (allOutputItems ? allOutputItems.length : 1)) %
  ...
  return (
    <>
            ...
            {allOutputItems?.map((outputItem, index) => {
              ...
              return (
                <div
                  key={index}
                  className={cn(
                    "flex border rounded-xl overflow-hidden cursor-pointer hover:bg-muted/50 transition-colors w-fit min-w-[160px] bg-white group transition-all duration-200 active:scale-95",
                ...
                >
                  <div
                    className={cn(
                      "bg-muted px-4 py-2 flex items-center justify-center border-r group-hover:bg-muted/80",
                      isActive ? "border-r-gray-500" : ""
                    )}
                  >
      ...
      <div
        className={cn(
          "flex flex-1 flex-col overflow-auto w-full p-4 pt-0 bg-background",
          currentItem.type === "table" ? "overflow-hidden" : ""
        )}
      >
        <div className="w-full max-w-full justify-center items-start">
          ...
          {currentItem.type === "text" && (
            <span className="w-full text-sm">
              <pre>{currentItem.content as string}</pre>
            </span>
          )}
